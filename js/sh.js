...
var playerHeight = Number(url.searchParams.get("altitude")) + 180;
...
function checkInput() {
  if (upPressed == true) {
    if (player.fuel > 0) {
      ...
      player.fuel += -0.07 * Number(url.searchParams.get("power"));
    }
  }
  if (leftPressed == true) {
    player.angularVelocity += -0.006*(player.rollAuth);
  }
...
function Particle() {
  ...
  if (explosion == false) {
    this.x = player.positionx + 5 - 95*Math.sin(radians);
    this.y = 95*Math.cos(radians) + 110;
  }
...
function drawPlayer(x, y, width, height, degrees) {
  ...
  draw.drawImage(document.getElementById("player"),-20, -50, 40, 160);
...
function drawText() {
  ...
  draw.fillText(target.height - Math.round(player.positiony) - 180, 10, 50);
...
function drawClouds() {
  drawCloud(cloud1.x, playerHeight - player.positiony - cloud1.y-180);
...
function shadeManipulate(value){
  /*var altitude = target.height - Math.round(player.positiony) - 180;
  if (altitude < 10000) {
...
function updateGameArea() {
  ...
  if (playing == true && landed == false) {
    ...
    if (-40*Math.cos(player.angle*Math.PI/180) + playerHeight - 180 - player.positiony < 0) {
    ...
    if (landed == true) {
      if (terrain == "land" || (Math.abs(player.positionx - (Math.round(((window.innerWidth)/2))-5 + targetOffset)) < 50)) {
        player.positiony = playerHeight - 180;
        ...
        drawFuelIndicator();
        drawParticles();
        if (target.height - Math.round(player.positiony) - 180 > -131) {
