...
var playerHeight = Number(url.searchParams.get("altitude")) + 150;
...
function checkInput() {
  if (upPressed == true) {
    if (player.fuel > 0) {
      player.velocityy += Math.cos(player.angle * Math.PI / 180)*(-1*(player.twr / 12));
      player.velocityx += -1 * Math.sin(player.angle * Math.PI / 180)*(-1*(player.twr / 12));
      player.fuel += -0.08 * Number(url.searchParams.get("power"));
    }
  }
  if (leftPressed == true) {
    player.angularVelocity += -0.007*(player.rollAuth);
  }
...
function Particle() {
  ...
  if (explosion == false) {
    this.x = player.positionx + 5 - 80*Math.sin(radians);
    this.y = 80*Math.cos(radians) + 100;
  }
...
function drawParticles() {
  ...
  for (var i = 0; i < particleSettings.density; i++) {
    if (xbcontroller) {
      if (((upPressed == true || window.gp.buttons[7].value.toFixed(2) != 0) && player.fuel > 0 && landed != true)||(explosion == true && explosionParticles < 1000)) {
...
function drawPlayer(x, y, width, height, degrees) {
  ...
  draw.drawImage(document.getElementById("player"),-18, -50, 36, 140);
...
function drawText() {
  ...
  draw.fillText(target.height - Math.round(player.positiony) - 150, 10, 50);
...
function drawClouds() {
  drawCloud(cloud1.x, playerHeight - player.positiony - cloud1.y-150);
...
function shadeManipulate(value){
  /*var altitude = target.height - Math.round(player.positiony) - 150;
  if (altitude < 10000) {
...
function calculateDrag(coeff, velocity, functionDensity) {
  if (velocity < 0) {
    return -1 * Math.abs(0.002 * coeff * density * velocity * velocity);
  }
  else {
...
function calculateLift(angle, velocity) {
  var radians = angle * Math.PI / 180;
  return Math.sin(2*radians) * velocity * density * 0.015;
}
...
function updateGameArea() {
  ...
  if (playing == true && landed == false) {
    ...
    if (-40*Math.cos(player.angle*Math.PI/180) + playerHeight - 150 - player.positiony < 0) {
      if ((player.velocityy > 2)||(Math.abs(player.velocityx) > 0.6)||(player.angle > 6)) {
        console.log("RUD");
        playing = false;
      }
    ...
    if (landed == true) {
      if (terrain == "land" || (Math.abs(player.positionx - (Math.round(((window.innerWidth)/2))-5 + targetOffset)) < 50)) {
        player.positiony = playerHeight - 150;
        window.player.postUpdate();
        window.gameArea.clear();
        drawClouds();
        drawPlayer(player.positionx, 30, player.width, player.height, player.angle);
        drawGround(player.positiony-40, target.height);
        drawText();
        drawTarget();
        drawFuelIndicator();
        drawParticles();
        ...
        drawFuelIndicator();
        drawParticles();
        if (target.height - Math.round(player.positiony) - 150 > -131) {
