...
function Particle() {
  ...
  if (explosion == false) {
    this.x = player.positionx + 5 - 35*Math.sin(radians);
    this.y = 35*Math.cos(radians) + 80;
  }
...
function drawPlayer(x, y, width, height, degrees) {
  ...
  draw.drawImage(document.getElementById("player"),-20, -50, 40, 100);

/*
  // draw the rect on the transformed context
  // Note: after transforming [0,0] is visually [x,y]
  //       so the rect needs to be offset accordingly when drawn
  draw.rect(-width / 2, -height / 2, width, height - 15);
